...
if (!function_exists('pagination')) {
    function pagination($total, $pageIndex, $pageSize = 15, $url = '', $context = ['before' => 5, 'after' => 4, 'ajaxcallback' => '', 'callbackfuncname' => ''])
    {
        $pdata = [
            'tcount'  => 0,
            'tpage'   => 0,
...
if (!function_exists('pagination2')) {
    function pagination2($total, $pageIndex, $pageSize = 15, $url = '', $context = ['before' => 3, 'after' => 2, 'isajax' => false, 'ajaxcallback' => '', 'callbackfuncname' => ''])
    {
        ...
        if (!empty($total)) {
            ...
            if (1 < $pdata['tpage']) {
                ...
                if ($url) {
                    ...
                    $jump_get = $_GET;
                    $jump_get['page'] = '';
                    $pdata['jump'] = 'href="' . ($script_name ?? '') . '?' . http_build_query($jump_get) . $pdata['cindex'] . '" data-href="' . ($script_name ?? '') . '?' . http_build_query($jump_get) . '"';
                    $_GET['page'] = $pdata['findex'];
                    $pdata['faa'] = 'href="' . ($script_name ?? '') . '?' . http_build_query($_GET) . '"';
                    $_GET['page'] = $pdata['pindex'];
                    $pdata['paa'] = 'href="' . ($script_name ?? '') . '?' . http_build_query($_GET) . '"';
