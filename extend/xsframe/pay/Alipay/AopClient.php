...
use xsframe\exception\ThirdException;
use xsframe\pay\Alipay\Sign\EncryptParseItem;
use xsframe\pay\Alipay\Sign\SignData;
use xsframe\pay\Alipay\Util\AopEncryptUtil;
...
class AopClient
{
    ...
    public function __construct($gatewayUrl, $appId, $rsaPrivateKey, $alipayrsaPublicKey, $encryptKey, $signType)
    {
        ...
        //            $this->clientAop->gatewayUrl         = $this->gatewayUrl;
        //            $this->clientAop->appId              = $this->appId;
        //            $this->clientAop->rsaPrivateKey      = $this->rsaPrivateKey;
        //            $this->clientAop->alipayrsaPublicKey = $this->rsaPublicKey;
        //            $this->clientAop->signType           = 'RSA2';
    }
    ...
    protected function curl($url, $postFields = null)
    {
        ...
        if (curl_errno($ch)) {
            throw new ThirdException(curl_error($ch), 0);
        } else {
            $httpStatusCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            if (200 !== $httpStatusCode) {
                throw new ThirdException($reponse, $httpStatusCode);
            }
        }
     ...
     *
     * @return 提交表单HTML文本|string 构建好的、签名后的最终跳转URL（GET）或String形式的form（POST）
     * @throws \Exception
     */
    public function pageExecute($request, $httpmethod = "POST")
    {
        ...
        if (strcasecmp($this->fileCharset, $this->postCharset)) {
            ...
            throw new ThirdException("文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!");
        }
        ...
        if (method_exists($request, "getNeedEncrypt") && $request->getNeedEncrypt()) {
            ...
            if ($this->checkEmpty($apiParams['biz_content'])) {
                ...
                throw new ThirdException(" api request Fail! The reason : encrypt request is not supperted!");
            }

            if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

                throw new ThirdException(" encryptType and encryptKey must not null! ");
            }

            if ("AES" != $this->encryptType) {

                throw new ThirdException("加密类型只支持AES");
            }

            // 执行加密
            $enCryptContent           = AopEncryptUtil::encodeSslAESBase64($apiParams['biz_content'], $this->encryptKey);
    ...
    public function checkResponseSign($request, $signData, $resp, $respObject)
    {
        ...
        if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
            if ($signData == null || $this->checkEmpty($signData->sign) || $this->checkEmpty($signData->signSourceData)) {
                throw new ThirdException(" check sign Fail! The reason : signData is Empty");
            }
            // 获取结果sub_code
            ...
            if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
                ...
                if (!$checkResult) {
                    if (strpos($signData->signSourceData, "\\/") > 0) {
                        ...
                        $checkResult              = $this->verify($signData->signSourceData, $signData->sign, $this->alipayPublicKey, $this->signType);
                        if (!$checkResult) {
                            // $this->logger->error("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
                            throw new ThirdException("check sign Fail! ");
                        }
                    } else {
                        // $this->logger->error("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
                        throw new ThirdException("check sign Fail!");
    ...
    private function encryptJSONSignSource($request, $responseContent)
    {
        ...
        $bizContent = AopEncryptUtil::decodeSslAESBase64($parsetItem->encryptContent, $this->encryptKey);
